            ...
            <CardHeader className="pb-4 md:pb-6">
              <div className="flex flex-col md:flex-row items-center md:items-start gap-4 md:gap-6">
                ...
                <Avatar className="h-20 w-20 md:h-24 md:w-24 ring-4 ring-slate-100">
                  <AvatarFallback className="text-2xl md:text-3xl font-bold bg-gradient-to-br from-blue-500 to-blue-700 text-white">
                    {initial}
                  </AvatarFallback>
                </Avatar>
                ...
                {statsData.map((stat, index) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="group rounded-lg border border-slate-200 bg-slate-50/50 p-3 md:p-4 transition-all hover:border-slate-300 hover:bg-white hover:shadow-sm"
                    >
                      <div className="flex items-center gap-2 md:gap-3">
                        <div className={`flex h-8 w-8 md:h-10 md:w-10 items-center justify-center rounded-lg ${stat.bgColor}`}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm md:text-lg font-semibold text-slate-900 truncate">
                            {loading ? "..." : stat.value}
                          </p>
                        </div>
                      </div>
          ...
          <Card>
            ...
            <CardContent>
              <dl className="space-y-3 md:space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-1 md:gap-4 py-2 md:py-3 border-b border-slate-100">
                  <dt className="text-xs md:text-sm font-medium text-slate-500">Nama Tampilan</dt>
                  <dd className="md:col-span-2 text-sm md:text-base text-slate-900">{displayName}</dd>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-1 md:gap-4 py-2 md:py-3 border-b border-slate-100">
                  <dt className="text-xs md:text-sm font-medium text-slate-500">Email</dt>
                  <dd className="md:col-span-2 text-sm md:text-base text-slate-900">{email}</dd>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-1 md:gap-4 py-2 md:py-3 border-b border-slate-100">
                  <dt className="text-xs md:text-sm font-medium text-slate-500">Tipe Akun</dt>
                  <dd className="md:col-span-2 text-sm md:text-base text-slate-900">
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                      Gratis
                    </span>
                  </dd>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-1 md:gap-4 py-2 md:py-3">
                  <dt className="text-xs md:text-sm font-medium text-slate-500">Member Sejak</dt>
                  <dd className="md:col-span-2 text-sm md:text-base text-slate-900">
